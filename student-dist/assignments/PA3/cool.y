...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <expression>  let_declaration

    %type <cases>       case_declarations
    ...
    */
    ...
    | OBJECTID '(' expression_list ')'
        { Entry *self = idtable.add_string("self"); $$ = dispatch(object(self), $1, $3); }
    ...
    | LET let_declaration
        { $$ = $2; }
    | CASE expression OF case_declarations ESAC
        { $$ = typcase($2, $4); }
    | NEW TYPEID
        { $$ = new_($2); }
    | ISVOID expression
    ...
    | OBJECTID ':' TYPEID ASSIGN expression IN expression %prec LET_PREC
        { $$ = let($1, $3, $5, $7); }
    /* If error is in last let assignment */
    | OBJECTID ':' TYPEID ASSIGN error IN expression %prec LET_PREC
        {  }
    | OBJECTID ':' TYPEID ',' let_declaration
        { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN expression ',' let_declaration
        { $$ = let($1, $3, $5, $7); }
    /* If error is in middle of let assignments */
    | OBJECTID ':' TYPEID ASSIGN error ',' let_declaration
        {  }
    ;
    ...
    case_declarations:
      case_expression ';'
        { $$ = single_Cases($1); }
    | case_declarations case_expression ';'
